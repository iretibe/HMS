...
namespace HMS.WPF.ViewModels
{
    public class ResidentPatientDetailsViewModel : BaseViewModel
    {
        ...
        public ResidentPatientDetailsViewModel(String id)
        {
            ...
            foreach (Room room in Hospital.Rooms.Values)
            {
                if (room.hasAvailableBed())
                    PatientRoomNumberComboBox.Add(new ComboBoxPairs(room.RoomID, room.RoomNumber.ToString()));
            }
            ...
            {
                DoctorsList.Add(new ComboBoxPairs(doctor.PersonID, doctor.Name));
            }
            ...
            {
                NursesList.Add(new ComboBoxPairs(nurse.PersonID, nurse.Name));
            }
            ...
            {
                MedicalHistoryList.Add(new ComboBoxPairs(medicine.MedecineID, medicine.Name + " - Starting Date: " + medicine.StartingDate.ToShortDateString() + " | " + medicine.EndingDate.ToShortDateString()));
            }
            ...
            foreach (Employee employee in Hospital.Employees.Values)
            {
                ...
                if (employee.GetType() == typeof(Doctor) && valid)
                {
                    ...
                    {
                        if (employee.PersonID == invalidEmployee.PersonID)
                        {
                            valid = false;
                        }
                    }
                    ...
                    if (valid)
                        DoctorsComboBox.Add(new ComboBoxPairs(employee.PersonID, employee.Name));
                }
            }
        ...
        public void AssignDoctor()
        {
            ...
            {
                if (doctor.PersonID == DoctorComboBox.Key)
                {
                    Hospital.Patients[PatientID].Doctors.Add(doctor.PersonID, doctor);
                    HospitalDB.InsertDoctorPatient(doctor.PersonID, PatientID);
        ...
        public void AddMedicine()
        {
            ...
            ((ResidentPatient)Hospital.Patients[PatientID]).History.Add(medicine.MedecineID, medicine);
            Medicine tempMedicine = ((ResidentPatient)Hospital.Patients[PatientID]).History[medicine.MedecineID];
            MedicalHistoryList.Add(new ComboBoxPairs(medicine.MedecineID, medicine.Name + " - Starting Date: " + medicine.StartingDate.ToShortDateString() + " | " + medicine.EndingDate.ToShortDateString()));
