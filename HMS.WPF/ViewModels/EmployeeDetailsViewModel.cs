...
namespace HMS.WPF.ViewModels
{
    public class EmployeeDetailsViewModel : BaseViewModel
    {
        ...
        public EmployeeDetailsViewModel(String id)
        {
            ...
            if (Hospital.Employees[EmployeeID].Department != null)
                EditEmployeeDepartment = new ComboBoxPairs(Hospital.Employees[EmployeeID].Department.DepartmentID, Hospital.Employees[EmployeeID].Department.Name);
            ...
            if (Hospital.Employees[id].GetType() == typeof(Doctor))
            {
                ...
                {
                    PatientsList.Add(new ComboBoxPairs(patient.PersonID, patient.Name));
                }
                ...
                {
                    AppointmentsList.Add(new ComboBoxPairs(appointment.AppointmentID.ToString(), ("Date: " + appointment.Date.ToString() + " | " + appointment.Patient.Name)));
                }
            ...
            else
            {
                ...
                {
                    RoomsList.Add(new ComboBoxPairs(room.RoomID, room.RoomNumber.ToString()));
                }
            ...
            foreach (Patient patient in Hospital.Patients.Values)
            {
                ...
                if (Hospital.Employees[EmployeeID].GetType() == typeof(Doctor))
                    ...
                    {
                        if (patient.PersonID == invalidPatient.PersonID)
                        {
                            valid = false;
                        }
                    }
                ...
                if (valid)
                    PatientsComboBox.Add(new ComboBoxPairs(patient.PersonID, patient.Name));
            }
            ...
            foreach (Room room in Hospital.Rooms.Values)
            {
                ...
                if (Hospital.Employees[EmployeeID].GetType() == typeof(Nurse))
                    ...
                    {
                        if (room.RoomID == invalidRoom.RoomID)
                        {
                            valid = false;
                        }
                    }
                ...
                if (valid)
                    RoomsComboBox.Add(new ComboBoxPairs(room.RoomID, room.RoomNumber.ToString()));
            }

            foreach (Department department in Hospital.Departments.Values)
            {
                if (department != null)
                    EditDepartmentComboBox.Add(new ComboBoxPairs(department.DepartmentID, department.Name));
            }
        }
        ...
        public async void RemoveRoom()
        {
            if (Hospital.Employees[EmployeeID].GetType() == typeof(Nurse))
            {
                ...
                if (result.Equals(true))
                {
                    ...
                    {
                        ((Nurse)Hospital.Employees[EmployeeID]).Patients.Remove(patient.PersonID);
                        for (int i = 0; i < PatientsList.Count; i++)
                        {
                            if (PatientsList[i].Key == patient.PersonID)
                            {
        ...
        public void AssignPatient()
        {
            ...
            else
            {
                ...
                foreach (Patient patient in Hospital.Patients.Values)
                {
                    if (patient.PersonID == PatientComboBox.Key)
                    {
                        if (EmployeeRole == "Doctor")
                        {
                            ((Doctor)Hospital.Employees[EmployeeID]).Patients.Add(patient.PersonID, patient);
                            Hospital.Patients[patient.PersonID].assignDoctor(((Doctor)Hospital.Employees[EmployeeID]));
                            HospitalDB.InsertDoctorPatient(EmployeeID, patient.PersonID);
                            PatientsNumber = "Patients: " + ((Doctor)Hospital.Employees[EmployeeID]).Patients.Count.ToString();
                        }
                        else
                        {
                            ((Nurse)Hospital.Employees[EmployeeID]).Patients.Add(patient.PersonID, patient);
        ...
        public void AssignRoom()
        {
            ...
            foreach (Room room in Hospital.Rooms.Values)
            {
                if (room.RoomID == RoomComboBox.Key)
                {
                    Hospital.Rooms[RoomComboBox.Key].addNurse(((Nurse)Hospital.Employees[EmployeeID]));
                    ((Nurse)Hospital.Employees[EmployeeID]).Rooms.Add(room.RoomID, room);
                    HospitalDB.InsertNurseRoom(EmployeeID, room.RoomID);
